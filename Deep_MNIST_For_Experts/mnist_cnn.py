...
# We can now implement our first layer. It will consist of convolution, followed by max 
# pooling. The convolution will compute 32 features for each 5x5 patch. Its weight tensor 
# will have a shape of [5, 5, 1, 32]. The first two dimensions are the patch size, the next 
# is the number of input channels, and the last is the number of output channels. We will 
...
# corresponding to image width and height, and the final dimension corresponding to the number 
# of color channels.
...
# for the probability that a neuron's output is kept during dropout. This allows us to turn 
# dropout on during training, and turn it off during testing. TensorFlow's tf.nn.dropout op 
# automatically handles scaling neuron outputs in addition to masking them, so dropout just works 
